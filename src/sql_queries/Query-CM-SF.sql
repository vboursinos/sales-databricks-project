...
SELECT
    ...
    (SELECT ARRAY_AGG(s2.product_category)
     FROM sales s2
     WHERE s2.customer_id = s.customer_id  -- use the customer_id for correlation
       AND s2.sale_date BETWEEN '2023-01-01' AND '2023-01-31'
    ) AS product_categories,
    PARSE_JSON(s.sale_details) AS sale_details_json
FROM
    sales s
JOIN
